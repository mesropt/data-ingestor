...
- [ ] **Phase 11: Multi-Sheet Ingest** - On a multi-sheet workbook the human chooses what to ingest — one sheet or several merged — and the tool makes that choice informed rather than making it silently: it shows every sheet's signature, proposes the safe merge, names exactly how divergent sheets differ, and records which sheet every ingested row came from
...
**Goal**: On a multi-sheet workbook, **the human decides** what to ingest — one sheet, or several combined into one dataset — and the tool's job is to make that decision an informed one, never to make it silently. Today `_resolve_sheet` (`parsing/table.py:349`) picks exactly one sheet (ranking them structurally, asking only when no sheet wins clearly) and discards every other sheet without a word. That is right for a data sheet plus a legend, and wrong for a workbook holding one plate, one timepoint, or one batch per sheet. This phase surfaces every sheet with what the tool knows about it — row count, column signature, which sheets share a signature, which diverge and exactly how — proposes the safe merge, and lets the human choose. Merging divergent sheets stays possible, but only as an explicit, informed act. Nothing is ever merged or dropped on the tool's own initiative, and every ingested row remembers which sheet it came from.
...
**Success Criteria** (what must be TRUE):
  ...
  1. On a multi-sheet workbook the human is shown every sheet with what the tool knows about each, and chooses which to ingest — one sheet, or several merged. The tool never merges on its own. Picking a single sheet remains a first-class choice, and the existing single-sheet path does not regress. (SHEET-01)
  2. The tool proposes which sheets are safe to merge (matching column signatures) and names exactly how the others diverge. A human may still merge divergent sheets, but only after the tool states what happens to the mismatched columns. Nothing is merged, or dropped, silently. (SHEET-02)
