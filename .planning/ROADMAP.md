...
- [ ] 12-01-PLAN.md — Pure foundations: layout.py verdict types, unpivot.py transform, StructuralHint.layout round-trip (Wave A)
- [ ] 12-02-PLAN.md — The judge: one batched call per workbook, runtime-Literal wire model, index clamp, type-bucket redaction (Wave A)
- [ ] 12-03-PLAN.md — Parse-path dispatch: hint.layout drives parse(); key-value un-pivots into a real RawTable; gate unmoved (Wave A)
- [ ] 12-04-PLAN.md — Manifest onto the verdict: judge_fn seam, suppression re-keyed and widened, THE fails-on-main regression test (Wave B)
- [ ] 12-05-PLAN.md — API wiring: row_per_record null hypothesis, hint round trip with 422 boundary, SHAPE-04 captured-outbound proof both directions
- [ ] 12-06-PLAN.md — Frontend: sheet-card layout line + badges + disagree action; StructuralHintPanel becomes the one answer surface
- [ ] 12-07-PLAN.md — Wave C deletion: heuristic classifier removed, fail-closed unknown question, test-inventory closure, CSV gap recorded
