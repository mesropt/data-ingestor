...
1. **Exact adaptive-thinking token spend at `effort: "high"` for a 50-field prompt**
   ...
   - **Status (2026-07-10): RESOLVED — empirically, by one live billed call.** The orchestrator ran `uv run pytest tests/test_max_tokens_live.py` with real credentials immediately after plan 02-01 landed. The test passed in 33.6 s: a real 50-field `FieldSet` at `_MAX_TOKENS = 16000`, `thinking={"type": "adaptive"}`, `output_config={"effort": "high"}` returned `stop_reason != "max_tokens"`, a non-`None` `parsed_output`, and all 50 field mappings survived the wire→domain boundary. `_MAX_TOKENS = 16000` is therefore validated at the D-04 cap, not merely estimated. The remaining uncertainty is the *margin*: the test asserts non-truncation, not headroom, so it cannot say how close 50 fields came to the ceiling. Should the cap ever rise above 50, re-run this probe rather than reasoning about it statically.
...
- Pitfalls: HIGH for decimal-comma/date/token-budget pitfalls (all grounded in real corpus data or real computed payload sizes). The adaptive-thinking token-spend estimate (Open Question 1) was MEDIUM at research time because protocol forbade a live billed call; it was raised to HIGH on 2026-07-10 when the probe was run for real during execution and `_MAX_TOKENS = 16000` held at 50 fields.
