...
### Decisions taken after research (2026-07-13) — these close every open item above

Research (`12-RESEARCH.md`) measured the driving workbook end to end and settled the four discretionary questions. It also found the phase's stakes are **worse than D-12-01 said**, and raised one product call. All are locked here.

- **D-12-12 (the leak is LIVE, not hypothetical — this corrects D-12-01):** D-12-01 says *"only low header confidence stopped a patient's name shipping as a column header."* **It did not stop it.** `_reportable_headers` (`sheets.py:255-268`) suppresses on `UNSUPPORTED_SHAPE` **only, and deliberately not on `HEADER_UNCERTAIN`** (that exemption is what keeps meridian's LEGEND working). `Patient Info` is `header_uncertain` — so today it reports `headers = ['Name', 'TAYLOR, James', '', 'Accession #', 'CS-2026-698392']`, and `service.py:2184` hashes that list into the **`column_signature` that keys the learning store**. `Methodology & Notes` does the same. The Python classifier gets **3 of 8 sheets right** on this workbook, and one of the three (`Result Visualization`, a chart sheet) is `ok` only because a title cell defeats `is_drawing_only_sheet`. **Consequence for the plan:** the regression test that asserts `"TAYLOR, James"` is absent from the manifest headers **must fail on `main`** before it passes. A test that is already green proves nothing.

- **D-12-13 (the verdict contract — one label column is not enough):** both driving sheets carry **two side-by-side key-value blocks belonging to ONE record** (labels col 0 / values col 1, *and* labels col 4 / values col 5). A verdict with a single `label_column: int` is wrong on the very first file. The contract is a new pure module `parsing/structure/layout.py`: `LayoutKind` (`row_per_record` / `key_value` / `wide_matrix` / `multiple_tables` / `not_a_table` / `unknown`), and a `SheetLayout` carrying a **tuple of `KeyValueBlock(label_column, value_columns, first_row, last_row)`** plus `one_record_per_value_column`, `header_row_index`, `first_data_row`, `last_data_row`, `confidence`, `reasoning`. Two things fall out for free and are hereby **taken**: (a) a single block with `value_columns=(1,2,3)` **is** the transposed layout, so the discretionary "does the un-pivot cover transposed?" is answered **yes**; (b) `first/last_data_row` makes `Quality Control` — a real 3-row table today refused because trailing Westgard prose trips `multiple_tables` — **ingestible**. Every field is an `int`, a `bool`, an enum or free-text *reasoning*: the wire model **structurally has nowhere to put a cell value**, which is how D-12-03/D-12-10 stop being a promise and become a type.

- **D-12-14 (one call per workbook, not one per sheet):** measured, the whole 8-sheet workbook is **~1,874 evidence tokens** (~1,419 redacted) — ~2.3k with the system prompt. The cost is **latency, not dollars**: eight sequential `opus` calls at `effort: high` would sit in front of the sheet screen. The judge takes the whole workbook in **one** call and returns one verdict per sheet, with `sheet_name` built as a **runtime `Literal` over the workbook's real sheet names** (the device `schema_ranker.py:76-91` already uses), so the model cannot invent a sheet. `_to_domain` **clamps every index against the real grid** and **fills any sheet the model omitted with `UNKNOWN`** — a forgotten sheet must ask, never default to `row_per_record`. Merging the judge into the existing `schema_ranker` call is **rejected**: the ranker is deliberately the last rung of the Python→Claude→human ladder (`service.py:1935-1946`), and merging would invert it.

- **D-12-15 (`row_per_record` is the null hypothesis — this is the product call research raised):** D-12-02 assumed the sheet screen is always there to confirm on. **It is not** — `upload.py:262` only raises it for `>1` worksheet, and 9 of 30 corpus workbooks are single-sheet. The rule: **a confident `row_per_record` verdict proceeds with no question** — which is *exactly* today's behaviour (`table.py:363-365` auto-applies its own `row_per_record` with no human in the loop). **Every other verdict** — `key_value`, `wide_matrix`, `multiple_tables`, `not_a_table`, `unknown`, or a low-confidence `row_per_record` — returns a `StructureQuestion` with **`answerable_by_hint=True`**, confirmed on the `StructuralHintPanel` that already exists. The principle, stated so it can be argued with: **`row_per_record` means "read it the ordinary way" and changes no value's meaning; every other verdict changes what a value *is*, and must be confirmed by a human.** Rejected: making the sheet screen universal (`>= 1`) — it forces a sheet-selection click on every single-sheet upload for zero gain, breaks `tests/api/test_sheets_route.py:554`, and still leaves CSVs uncovered.
  This also finally makes `answerable_by_hint=True` **mean** something: `hint.table_shape` is **write-only today** (`grep` finds two writes, zero reads) — which is precisely why the current shape question is marked unanswerable. The verdict is what turns that dead end into a round trip.

- **D-12-16 (fail closed, and say out loud how it differs from the ranker):** no verdict — no API key, auth error, rate limit, outage, malformed response, hallucinated index, omitted sheet — ⇒ `UNKNOWN` ⇒ a `StructureQuestion` the human **can now answer**. Copy `service._rank_or_none`'s broad-`except`-at-an-availability-boundary idiom (`service.py:1975-1994`) **and state the difference in the code, because the two look identical and are not**: a *ranker* failure costs a **suggestion** (the human picks a Schema); a *judge* failure costs a **question** (the human is asked about the layout). Neither guesses. Accepted cost: when the model is unreachable, a multi-sheet workbook's sheet screen becomes all questions — worse UX than today's 3-of-8 `ok`, but today's 3-of-8 includes a chart sheet offered as ingestible. Degraded-but-honest is the price of D-12-07/D-12-08, already accepted.

- **D-12-17 (the eval set does not exist — build it, and measure `headers_only`'s real cost):** `manifest.json`'s `layout` label is **per-file, not per-sheet**, covers **20 of 30** workbooks, and **does not include the driving file** — so it cannot grade this phase. Author `data/synthetic/lab_corpus/layout_truth.json`: per-sheet ground truth over the 12 multi-sheet corpus workbooks plus every `data/synthetic/` shape fixture (~50 of the 141 worksheets), declared in-file as a **sample, not a census**. Gates, **measured under `ASSAYINGEST_LIVE_TESTS=1` and reported in VERIFICATION.md, not asserted**: `key_value` recall **100%** on the default grid (a missed key-value sheet is the exact silent failure this phase exists to kill), `row_per_record` precision **≥95%** (a false `key_value` is loud and human-correctable — the safer error), and **zero** confidently-wrong verdicts (confidence ≥0.9 and wrong). For the **redacted** grid: **measure and report — set no bar before the number exists.** If redacted recall lands low, the honest fix is to **ask the human more often in `headers_only`**, never to weaken the redaction (D-12-11).
  Redaction detail: bucket string lengths (`str:short` ≤8 / `str:med` 9-24 / `str:long` 25+) rather than emitting `str(13)` — a raw length is a real side channel, and the bucket keeps all the discriminative power (labels cluster short, free text clusters long).

- **D-12-18 (scope fence — CSVs are out, and it is said out loud rather than discovered later):** `classify_shape` **never ran on CSVs** (`_parse_csv_structurally`, `table.py:242-262`, has no shape gate), so removing it cannot regress them. A key-value **CSV** parses into garbage today and **will continue to** after this phase. Judging CSVs is nearly free in code but adds a Claude call to every CSV upload — the CLI's main path and the path most tests run with no credentials. Out of scope; recorded as a known gap and a follow-up todo.

### Claude's Discretion (all four now settled — kept for the record)

- ~~The exact evidence-grid bounds~~ → **20×10, text-rendered with visible row/column indices** (the model must *see* the indices it returns), plus the sheet's true dimensions so a truncated view is never mistaken for the whole sheet (the trap `mapper.py:158-160` already avoids). D-12-14.
- ~~`TableShape` gains `KEY_VALUE`, or a richer object?~~ → **a richer object.** D-12-13.
- ~~Does the un-pivot cover transposed for free?~~ → **yes, taken.** D-12-13.
- ~~How is `classify_shape`'s removal sequenced?~~ → **three waves: A add (nothing removed, 1132 still green) → B switch the manifest onto the verdict (the regression test that fails on `main` lands here; suppression is only ever *widened*) → C delete `shape.py` and the fallback.** Only after B proves the verdict path works end to end may the fallback disappear.
